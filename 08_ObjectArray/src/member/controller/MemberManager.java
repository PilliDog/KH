...
public class MemberManager {
	...
	public void insertUser() {
			...
			{
				new MemberManager().insertSilver();
			}
			else if(userRate.equals("gold"))
			{
				new MemberManager().insertGold();
			}
			else if(userRate.equals("vip"))
			{
				new MemberManager().insertVip();
			}
			else
			{
				new MemberManager().insertVvip();
			}
	...
	public void insertSilver() {
		...
		customer_silver[countSilver] = new Silver();
		customer_silver[countSilver].setRate("silver");
		System.out.print("이름을 입력해주세요 : ");
		customer_silver[countSilver].setName(sc.nextLine());
		System.out.print("포인트를 입력해주세요 : ");
		customer_silver[countSilver].setPoint(sc.nextInt());
		customer_silver[countSilver].setBonus((customer_silver[countSilver].getPoint()*0.02));	
		System.out.println();
		countSilver ++;
	}
